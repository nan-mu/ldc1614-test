...
设置参考计数的转换间隔，$Conversion\ Time$，$t_{cx}$。

该函数的作用是向 `0x08 + (通道对应偏移)` 写入一个 `u16`。在[文档](https://www.ti.com/lit/ds/symlink/ldc1614.pdf#page=20)中对应寄存器 `RCOUNTx`。在[文档](https://www.ti.com/lit/ds/symlink/ldc1614.pdf#page=39)中对哦于转换间隔的相关描述为：

> LDC1612/LDC1614 通过设置内部寄存器提供可配置的转换间隔。转换间隔可以配置在 $1.2 \mu s$ 到 $>26.2 ms$ 的范围内，分辨率为 16 位。请注意，可以将转换间隔配置得显著短于读取 DATAx 寄存器所需的时间；在这种配置下，当每个通道的新转换数据完成时，旧的转换数据会被覆盖。转换间隔通过设置 RCOUNTx 寄存器的值，以参考时钟周期的倍数进行设置。

其中提到的设置计算公式为（x为寄存器`RCOUNTx`的值）：

$$
t_{cx}(x) = \frac{x\times16+4}{f_{REFx}}
$$

不过一般来说，较长的转换间隔将提供更高分辨率的电感测量。全分辨率需要最大设置 0xFFFF。而在[文档](https://www.ti.com/lit/ds/symlink/ldc1614.pdf#page=51)中，推荐设置为0x04D6.
...
设置线圈驱动电流振幅，$Sensor Maximum Current drive$，$I_{SENSORMAX}$。

该函数将在`DRIVE_CURRENTx`寄存器，地址为`0x1E + (通道对应偏移)`，写入`cur`，用于控制传感器驱动电流，使传感器信号幅度在 $1.2 V$ 至 $1.8 V$ 的范围内。，`cur`的取值应当根据[文档](https://www.ti.com/lit/ds/symlink/ldc1614.pdf#page=42)的表42进行。
...
其中一种就是调用上述方法配置寄存器`DRIVE_CURRENTx`，在这种方式下，必须禁用自动幅度校正且 `RP_OVERRIDE_EN`为`0b1`，`RP_OVERRIDE_EN`是地址为`0x1A`的寄存器`CONFIG`的第12位。

其中线圈驱动电流振幅（$Sensor Maximum Current drive$）的计算公式为：

$$
I_{ISENSORMAX} = \frac{\pi\times(V_{P})}{4R_{P}}
$$
...
2. 通过设置 `CONFIG` 将设备置于 `SLEEP` 模式。`SLEEP_MODE_EN` 到 `0b0`。
...
7. 从通道对应的相应的DRIVE_CURRENTx寄存器读取的 INIT_DRIVEx 字段（位 10：6）作为该通道的电流驱动值。保存此值。
